...
class PdoSqliteStorage extends Storage implements PdoAwareInterface
{
    ...
    public function __construct(
        ...
        if ($pdo->getAttribute(PDO::ATTR_DRIVER_NAME) !== 'sqlite') {
            throw new StorageException('PdoSqliteStorage only supports sqlite driver');
        }
        
        $pdo->sqliteCreateFunction('JSON_CONTAINS', static function ($json, $val, $path = null): bool {
            ...
            try {
                $value = json_decode($val, true, 512, JSON_THROW_ON_ERROR);
            } catch (JsonException|Throwable $e) {
                $value = $val;
            }
            ...
            if (is_array($jsonData)) {
                ...
                if (!is_null($path)) {
                    $path = ltrim($path, '$.');
                    $path = trim($path, '"');
                    $jsonData = Arr::get($jsonData, $path);
                }
                
                if (is_array($jsonData) && is_array($value)) {
                    return empty(array_diff($value, $jsonData));
                }
                
                if (is_array($jsonData) && array_is_list($jsonData)) {
                    return in_array($value, $jsonData);
                }
                
                if (is_array($jsonData)) {
                    return empty(array_diff_assoc([$value], $jsonData));
                }
    ...
    {
        $grammar = new PdoSqliteGrammar($this->tables())
            ->select($this->select)
            ->table($this->table ?? '')
            ->joins($this->joins)
            ->wheres($this->wheres)
    ...
    public function insertItems(iterable $items, null|array $return = []): ItemsInterface
    {
        ...
        if ($this->skipQuery) {
            $this->grammar = (new PdoSqliteGrammar($this->tables()))
                ->table($table)
    ...
    public function update(array $item, null|array $return = []): ItemsInterface
    {
        ...
        if ($statement = $grammar->getStatement())
        {
            ...
            if (str_contains($statement, 'RETURNING')) {
                return new Items(items: $pdoStatement->fetchAll(PDO::FETCH_ASSOC), action: 'update');
            }
