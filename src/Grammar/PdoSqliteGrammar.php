...
class PdoSqliteGrammar extends Grammar
{
    /**
    ...
    protected function compileUpdateColumns(array $columns, array $values): string
    {
        ...
        foreach($columns as $column) {
            if ($column->jsonSegments()) {
                ...
                if (!isset($compiledJson[$column->name()])) {
                    $compiled[] = $this->backtickValue($column->name()).' = json_patch(ifnull('.$this->backtickValue($column->name()).', json(\'{}\')), json(?))';
                }
                
                $compiledJson[$column->name()] = true;
                
                // we remap values:
                $values[$column->name()] = Arr::set(
                    array: $values[$column->name()] ?? [],
                    key: implode('.', $column->jsonSegments()),
                    value: $values[$column->column()] ?? null
                );
                
                unset($values[$column->column()]);
            } else {
                $compiled[] = $this->backtickValue($column->name()).' = ?';
            }
        }
    ...
    protected function whereJsonContainsKey(array $where): null|string
    {
        ...
        return $where['boolean'].' json_type('.$column.', \'$.'.$path.'\') is not null';
    }
    ...
    protected function whereJsonLength(array $where): null|string
    {
        ...
        if (is_null($path)) {
            return $where['boolean'].' json_array_length('.$column.') '.$operator.' ?';
        }

        return $where['boolean'].' json_array_length('.$column.', \'$.'.$path.'\') '.$operator.' ?';
    }    
    ...
    protected function compileWhereColumn(ColumnInterface $column): string
    {
        ...
        if ($column->jsonSegments()) {
            ...
            return 'json_extract('.$compiled.', \'$.'.$path.'\')';
        }
